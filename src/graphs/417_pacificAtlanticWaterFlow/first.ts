...
Runtime 1038ms beats 7.25% of users with TypeScript
Memory 76.68MB beats 6.52% of users with TypeScript
*/
...
const pacificAtlantic = (heights: number[][]): number[][] => {
  ...
  const dfs = (row: number, column: number, visited = new Set<string>()) => {
    ...
    while (stack.length) {
      ...
      // if (reachedAtlantic)
      // console.log(`   '${sRow},${sColumn}' reaches atlantic`);
      ...
      // if (reachedPacific)
      // console.log(`   '${sRow},${sColumn}' reaches pacific`);
